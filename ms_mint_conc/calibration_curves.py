...
def find_linear_range_variable_slope_interval(x, y, th, interval):
    ...
    x_c = x
    y_c = y
    y_intercept, slope, res, r_ini, r_last = classic_lstsqr_variable_slope_interval(
        x_c, y_c, interval
    )
...
def calibration_curves(x_train, y_train):
    ...
    for col in calibration_curves.peak_label:
        ...
        #             print(min(x_c))
        calibration_curves.lin_range_min[calibration_curves.peak_label == col] = min(
            y_c
        )
        calibration_curves.lin_range_max[calibration_curves.peak_label == col] = max(
            y_c
        )
    ...
    calibration_curves["LLOQ"] = calibration_curves.lin_range_min.apply(
        lambda x: np.exp(x)
    )
    calibration_curves["ULOQ"] = calibration_curves.lin_range_max.apply(
        lambda x: np.exp(x)
    )
...
def calibration_curves_variable_slope_interval(x_train, y_train, interval):
    ...
    for col in calibration_curves.peak_label:
        ...
        x = np.log(x)
        if len(x > 2):
            y_inter, slope, x_c, y_c, res = find_linear_range_variable_slope_interval(
                x, y, 0.01, interval
            )
        #             print(min(x_c))
        calibration_curves.lin_range_min[calibration_curves.peak_label == col] = min(
...
def info_from_Mint_dense(mint_):
    ...
    out_df = mint_.melt(
        id_vars=["peak_label"], var_name="ms_file", value_name="peak_max"
    )
...
def setting_from_stdinfo(std_info, results_):
    ...
    try:
        output.ms_file = output.ms_file.apply(
            lambda x: os.path.basename(x).replace(".mzXML", "")
        )
    except:
        pass
    ...
    for file in np.unique(output.ms_file):
        for cp in np.unique(output.peak_label):
            try:
                #                 print(file)
                output.STD_CONC[
                    (output.ms_file == file) & (output.peak_label == cp)
...
def mint_train_set(mint_results, by="peak_max"):
    ...
    X_train = mint_results[mint_results.ms_file.str.contains("Standard")].set_index(
        "ms_file"
    )[["peak_label", by]]
...
def transform(X, calibration_curves):
    calibration_curve = calibration_curves[
        ["peak_label", "slope", "intercept", "lin_range_min", "lin_range_max"]
    ]
    X0 = X.copy().fillna(0)
    r_min = 0
    r_max = 0
